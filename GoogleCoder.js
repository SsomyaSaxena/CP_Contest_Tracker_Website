...
var Event = mongoose.model("GC",eventSchema,"gc");
...
var URL = "https://clist.by/api/v1/json/contest/?limit=1000&resource__name=codingcompetitions.withgoogle.com&start__gte=2020-07-01T00%3A00%3A00&username=Ssomya&api_key=c951303aa1d1a71d9baab5d7570a79d9f35de3a0";
    ...
    const result = await Event.findOne({eventId: response.data.objects[i].id}) 
      if(result==null)
      {
        // console.log("Lets create an event!");
         ...
         description: String(response.data.objects[i].href)+' - CodingCompetitionsWithGoogle',
         start:{
       ...
       await calendar.events.insert({calendarId : 'i1nh34g3doa9a5i9md2esbnb6g@group.calendar.google.com',resource: event}, err => {
         if(err)
       ...
       if (err) 
       {
        console.log("ERROR IN SAVING EVENT ID TO DB GC "+err);
       }  
