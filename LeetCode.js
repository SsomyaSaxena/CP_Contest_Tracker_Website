...
var Event = mongoose.model("LC",eventSchema,"lc");
...
var URL = "https://clist.by/api/v1/json/contest/?limit=1000&resource__name=leetcode.com&start__gte=2020-07-1T00%3A00%3A00&username=Ssomya&api_key=c951303aa1d1a71d9baab5d7570a79d9f35de3a0";
    ...
    const result = await Event.findOne({eventId: response.data.objects[i].id}) 
      if(result==null)
      {
        // console.log("Lets create an event!");
         ...
         description: String(response.data.objects[i].href)+' - LeetCode',
         start:{
       ...
       await calendar.events.insert({calendarId : '9d16378v9e7dav2ij2dv6uje44@group.calendar.google.com',resource: event}, err => {
         if(err)
       ...
       if (err) 
       {
        console.log("ERROR IN SAVING EVENT ID TO DB LC "+err);
       }  
